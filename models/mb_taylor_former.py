## Modified from: https://github.com/FVL2020/ICCV-2023-MB-TaylorFormer
...
def mb_taylor_former_b():
    config = OmegaConf.load("configs/base/mb_taylor_former_b.yaml")
    _ = config['network_g'].pop('type')
    
    normalize = tfs.Compose([tfs.Lambda(lambda img: img * 0.5 + 0.5)])
    
    model_restoration = MB_TaylorFormer(**config['network_g'], normalize=normalize)
...
class MB_TaylorFormer(nn.Module):
    ...
    def forward(self, inp_img):
        ...
        # [0, 1] -> [-1, 1]
        out_dec_level1 = 2 * out_dec_level1 - 1
